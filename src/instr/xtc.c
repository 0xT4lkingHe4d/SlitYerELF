...
void _xtc_build_instr(__u8 sc[15], __u16 f) {
	_build_instr(&x86_64_tabl[f & 0x3ff], sc + xtc_set_ext(sc, f >> 12));
}
...
__s8 xtc_make_in(instr_dat_t *dst, __u8 flags[4], void *ptr, const instr *in) {
	...
	if (init_instr(X86_64, dst, ptr) == -1) return -1;
...
__s16 xtc_chng_instr_sz(instr *in, __u8 oper[4]) {
	...
	__s16 ndx = __get_tabl_ndx(X86_64, in);
	for (xtc_st *i = map; i <= &map[mm.size/sizeof(xtc_st)]; i++)
...
static __s8 x_t_c_insert(xtc_st *st, __u16 sub_ndx, instr *in) {
	if (!st) return -1;
	st->t_ndx = __get_tabl_ndx(X86_64, (in));
	st->c_pos = sub_ndx;
	...
	for (__u8 l = 0; l < 0b111; l++) {
		...
		instr_dat_t ins = {0};
		if (init_instr(X86_64, &ins, sc) == -1) continue;
...
mmsz_t *x_t_c_init() {
	...
	__u64 map_sz = MAX_TABLE_NDX(X86_64) * sizeof(xtc_st);
	xtc_st *map = map_anon(map_sz);
	__u64 map_ndx = 0;

	const instr *in = NULL, *_in = NULL;
	__u64 ndx = 0, i = 0;
	__u8 b_map[MAX_TABLE_NDX(X86_64)] = {0};
