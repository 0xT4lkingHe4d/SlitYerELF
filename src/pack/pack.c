...
u64 pack_hdr_align(pack_t *p, __u64 virt);
...
void elf_shift_off(elf_t *elf, __u64 off, __u64 sz);
...
void pack_insert(pack_t *pack, __u64 off, struct mmsz_tup *mm) {
	...
	for (off_mmsz_t *i = NULL; !!(i = x_get_min(pack->hdr.ptr, pack->hdr.num, i)); ) {
		if (_contain_(i->off, i->mm->sz, off)) {
			_hexdump(mm->mem, mm->sz);
		}
	}

	p_add_patch(pack, mm->mem, off, mm->sz);
}
...
__u64 pack_fix_entry(struct mmsz_tup *new, struct mmsz_tup *old, __u64 ptr) {
	return (_contain_(old->mem, old->sz, ptr)) ? (new->mem + ((void*)ptr - (void*)old->mem)) : ptr;
}
...
void patch_elf(pack_t *p) {
	...
	*entry = pack_vtov(p, p->orig_elf, *entry);
	...
	pack_fix_st *st = NULL;
	ll_each(&p->modif.ll_fixes, st) {
		*(__u64*)st->dst = st->rel->out.off + pack_hdr_align(p, st->rel->out.off)
							+ st->off + pin_align(p, 0, st->off);
	}
...
u64 pack_hdr_align(pack_t *p, __u64 v) {
	u64 ret = 0;
...
lle *pack_fix_u64(pack_t *p, rel_patch_t *rel, void *dst, __u64 off) {
	...
	lle *e = llist_add(&p->modif.ll_fixes, F_LL_MALLOC, &st, sizeof(pack_fix_st));
	return (!e) ? NULL : e->dat;
}