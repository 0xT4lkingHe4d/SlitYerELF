...
static __s8 import_plt(pack_t *p, merge_elf_t *m, __u8 *name);
	...
	*ptr = 0xcc;
	...
	foreach_instr_off(in, i, ptr + 1, m_fn->size) {
		...
		switch (p->p_flags) {
			...
			case PF_R:			// R--
				break;
				// merge_cpy_t *cpy = merge_cpy_get(m, sec->sh_offset);

				// if (!cpy) {
				// 	if (++m->mm_cpy_num * sizeof(merge_cpy_t) > m->mm_cpy.sz) {
				// 		resz_mmsz(&m->mm_cpy, 0x10 * sizeof(merge_cpy_t));
				// 	}
				// 	cpy		= &((merge_cpy_t*)m->mm_cpy.mem)[m->mm_cpy_num];
				// 	*cpy	= (merge_cpy_t){
				// 		.ok = 1,
				// 		.s_off = sec->sh_offset,
				// 		.d_off = s_sec->sh_offset + s_sec->sh_size,
				// 	};

				// 	prf("%lx\n", cpy->d_off);

				// 	pack_patch(pack, PACK_ADD, cpy->d_off, m_elf->map + sec->sh_offset, sec->sh_size);
				// }

				// _rel_hijack_(rel, imm, cpy->d_off + (imm - sec->sh_addr));
			default:
				return -1;
		}
	}
...
static __s8 m_instr_imm_X(pack_t *p, merge_elf_t *m, __u64 imm, me_target_t *trg, __u64 off) {
	...
	if (plt_rel->t == DT_RELA) {
		...
		__u64 off = import_plt(p, m, m_name);
		if (off == -1) return -1;
...
static __s8 import_plt(pack_t *p, merge_elf_t *m, __u8 *name) {
	...
	Elf_Shdr *got = elf_sec_by_name(&p->elf, ".got.plt");
	if (!got) got = elf_sec_by_name(&p->elf, ".got");
	if (!got) return -1;

	__u64 got_addr = got->sh_addr + got->sh_size;
	__u64 *got_ptr = xmalloc(8);
	p_add_patch(p, got_ptr, pack_vtof(p, &p->elf, got_addr), 8);

	symtab_t *dyn	= elf_get_dyntab(&p->elf);
	rela_t *plt		= elf_get_plt_rela(&p->elf);
	if (!dyn || !plt) return -1;

	__u64 dynstr_sz = dyn->mm_str.sz;
	pack_ext_hdr(p, _elf_off(&p->elf, dyn->str), strlen(name) + 1);
	memcpy(dyn->str + dynstr_sz, name, strlen(name));


	__u64 di = _elf_symtab_sz(dyn) / sizeof(Elf_Sym);
	__u64 ri = _elf_rela_t_sz(plt) / sizeof(Elf_Rela);

	pack_ext_hdr(p, dyn->off, sizeof(Elf_Sym));
	dyn->tab[di] = (Elf_Sym){
		.st_info	= ELF64_ST_INFO(STB_GLOBAL, STT_FUNC),
		.st_name	= dynstr_sz,
	};

	pack_ext_hdr(p, plt->off, sizeof(Elf_Rela));
	plt->rela[ri] = (Elf_Rela){
		.r_info		= ELF64_R_INFO(di, R_X86_64_JUMP_SLOT),
		.r_offset	= got_addr,
	};

	pack_ext_hdr(p, p->elf.gnu_ver.sym_off, sizeof(__u16));
	p->elf.gnu_ver.sym[p->elf.gnu_ver.mm_sym.sz / sizeof(__u16) -1] = 3;
	...
	pack_fix_u64(p, m->s_rel, got_ptr, off + 6);

	*(__u32*)(mm->mem + 7)	= ri;
	*(__u32*)(mm->mem + 12)	= px->orig_off - sec_plt->sh_addr - 12;
	_rel_hook_(e->dat, HOOK_T_ORIG, 0, got_addr);
