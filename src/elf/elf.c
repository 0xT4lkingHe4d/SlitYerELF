...
static __s8 elf_init_versym(elf_t *elf) {
	...
	symtab_t *dyn	= elf_get_dyntab(elf);

	if (!!elf->dynamic.tab && !!dyn) {
		foreach_dynamic(elf, dyn) {
			__u64 v		= dyn->d_un.d_val;
			void *ptr	= elf->map + v;
			switch (dyn->d_tag) {
				case DT_VERSYM:		ver->sym	= ptr;	break;
				case DT_VERNEED:	ver->need	= ptr;	break;
				case DT_VERNEEDNUM:	ver->num	= v;	break;
			}
		}
		if (!!ver->sym) {
			ver->sym_off	= _elf_off(elf, ver->sym);
			ver->mm_sym.sz	= sizeof(__u16) * (_elf_symtab_sz(dyn) / sizeof(Elf_Sym));
		}
		return 0;
	}
	return -1;
